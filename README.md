...
> 当前状态：开始界面、三关完整流程、自定义模式、通关与失败结果页、道具机制、箭头飞出动画、碰撞反馈和音乐音效均已完成，当前正在完善提交材料。
...
- 退出游戏：点击右上角“退出游戏”按钮
- 自定义模式：从开始界面进入配置页，用长条滑块按行设置 1~10 的 N×N 棋盘、三类道具各 0~9 次，以及用于控制空位置比例的难度系数
- 自定义模式结算：清空棋盘后点击“再来一局”会重新随机生成；失败后点击“重新开始”也会重新随机生成；该模式没有下一关
- Esc：自定义配置页和游戏内均返回开始界面；在开始界面再次按下可退出游戏
...
├─ main.py                 # 程序入口
├─ game.py                 # 游戏主循环与状态管理
├─ custom_mode.py          # 自定义配置、长条滑块与随机棋盘生成
├─ ui.py                   # 纸张风格界面组件
├─ tools.py                # 道具定义与一次性状态
